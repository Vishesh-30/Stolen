      ...
      <main class="flex-grow container mx-auto px-6 py-6 text-left bg-gray-100">
        ...
        <div class="p-6 bg-white rounded-lg">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(stock, index) in stocks" :key="index" class="bg-white rounded-xl hover:shadow-lg hover:bg-green-50 p-6 flex flex-col items-center transition duration-300 ease-in-out cursor-pointer">
              <div class="mb-4">
      ...
      try {
        ...
        for (let date in data) {
          for (let ticker in data[date]) {
            ...
            const logoKey = logoMappings[name] || name.toLowerCase();
        ...
        transformedData.sort((a, b) => {
          return tickerOrder.indexOf(a.name) - tickerOrder.indexOf(b.name);
        });
